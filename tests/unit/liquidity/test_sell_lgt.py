...
def token_to_eth_input(eth_amount_to_buy: int) -> "Wei":
    """ Constant Price Model implementation with fee. """
    numerator = token_liquidity * eth_amount_to_buy * 1000
    denominator = (eth_liquidity - eth_amount_to_buy) * FEE_MODIFIER
...
def test_input_exceed_reverts(liquidLgt, accounts):
    with brownie.reverts("LGT: sell amount exceeds balance"):
...
def test_input_to_zero_reverts(liquidLgt, accounts):
    with brownie.reverts("dev: can't send to zero address"):
        liquidLgt.tokenToEthTransferInput(1, 1, 99999999999, ZERO_ADDRESS, {'from': accounts[5]})
...
def test_output_to_zero_reverts(liquidLgt, accounts):
    with brownie.reverts("dev: can't send to zero address"):
        liquidLgt.tokenToEthTransferOutput(1, 1, 99999999999, ZERO_ADDRESS, {'from': accounts[5]})
